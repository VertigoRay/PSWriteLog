...
.DESCRIPTION
    ...
    On first run, this script will install every version of PSWriteLog from the minimum version required to the latest version.
    ...
    PSWriteLog will also update the timestamp of it's current version each time the module is imported.
    ...
    Why go through all this trouble? I'm glad you asked, refer to the README:
        https://github.com/VertigoRay/PSWriteLog#advanced-start
#>
...
param (
    ...
    [Parameter(HelpMessage = 'Set the version that we started using PSWriteLog here. We know none of our scripts will use anything older than this.')]
    [version]
...
Start-Transcript -LiteralPath ([IO.Path]::Combine($env:SystemRoot, 'Logs', 'PSWriteLog-Remediation.log')) -IncludeInvocationHeader

[hashtable] $versionInstalled = @{
    LiteralPath = 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\com.github.VertigoRay\PSWriteLog\VersionsUsed'
...
if (Get-Module 'PSWriteLog' -ListAvailable) {
    # PSWriteLog not currently installed.
    Update-Module 'PSWriteLog' -Force

    # Cleanup Old Versions
    Get-Module 'PSWriteLog' -ListAvailable | Foreach-Object {
