...
. ('{0}\PSWriteLog\Public\Write-Progress.ps1' -f $psProjectRoot.FullName)

Describe 'Write-Progress' {
    BeforeAll {
        $ProgressPreference = 'SilentlyContinue'
        $script:DefaultLog = "${TestDrive}\Logs\Write-Progress.log"
    ...
    Context 'Write-Progress $Message' {
        BeforeAll {
            $script:DefaultLog = [IO.Path]::Combine($env:Temp, ('PowerShell {0} {1} {2}.log' -f $PSVersionTable.PSEdition, $PSVersionTable.PSVersion, $MyInvocation.CommandOrigin))
